...
class EchoWrist3DDemo:
        ...
        if gesture == '专注工作':
            ...
            for finger in fingers:
                for i in range(1, len(finger)):
                    fingers[finger][i][1] *= 0.7
                    fingers[finger][i][2] *= 0.8

        elif gesture == '压力状态':
            # 压力: 握拳
            for finger in fingers:
                for i in range(1, len(finger)):
                    fingers[finger][i][0] *= 0.6
                    fingers[finger][i][1] *= 0.4
                    fingers[finger][i][2] *= 0.5

        elif gesture == '疲劳状态':
            # 疲劳: 手部下垂
            for finger in fingers:
                for i in range(len(finger)):
                    fingers[finger][i][1] -= 1.5

        elif gesture == '放松状态':
            # 放松: 手指伸展
            for finger in fingers:
                for i in range(1, len(finger)):
                    fingers[finger][i][0] *= 1.3
                    fingers[finger][i][1] *= 1.1

        elif gesture == '创意思考':
            # 创意: 手部有活力
            for finger in fingers:
                for i in range(len(finger)):
                    fingers[finger][i][0] += np.sin(self.time_step * 0.1 + i) * 0.2
                    fingers[finger][i][1] += np.cos(self.time_step * 0.15 + i) * 0.15
    ...
    def draw_3d_hand(self, ax, fingers):
        ...
        palm_polygon = plt.Polygon(list(zip(palm_x, palm_y)),
                                 fill=True, alpha=0.3, color='pink', edgecolor='red', linewidth=2)
        ...
        for i, (finger_name, color) in enumerate(zip(finger_names, finger_colors)):
            points = fingers[finger_name.replace('拇指', 'thumb').replace('食指', 'index')
                                     .replace('中指', 'middle').replace('无名指', 'ring')
                                     .replace('小指', 'pinky')]
    ...
    def update_display(self, frame):
        ...
        self.ax_confidence.fill_between(confidence_theta, r_inner, r_outer,
                                       color='green' if self.confidence > 0.7 else 'orange', alpha=0.8)
    ...
    def run(self):
        ...
        # 创建动画
        anim = FuncAnimation(self.fig, self.update_display, interval=100, blit=False)
